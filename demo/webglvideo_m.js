...
function initBuffers() {
	...
	// texture
	videoTextures[0] = createTexture();
	videoTextures[1] = createTexture();
	videoTextures[2] = createTexture();
...
function drawScene() {
	...
	updateTexture(videoTextures[videoFrame%videoTextures.length], videoElement);
	if (videoFrame == 1) {
		updateTexture(videoTextures[0], videoElement);
	}
	...
	gl.bindTexture(gl.TEXTURE_2D, videoTextures[(videoFrame)%videoTextures.length]);
	gl.activeTexture(gl.TEXTURE0 + 1);
	gl.bindTexture(gl.TEXTURE_2D, videoTextures[(videoFrame+videoTextures.length-1)%videoTextures.length]);

	gl.uniform1iv(gl.getUniformLocation(shaderProgram, "u_samplers"), [0,1]);
